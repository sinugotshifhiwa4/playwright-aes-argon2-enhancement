...
// Updated main method - much shorter and focused
public async rotateKeyWithAudit(
  ...
  try {
    ...
    await this.recordRotationStartAudit(keyName, environmentFile, reason, customMaxAge, shouldRotateKey);
    ...
    logger.info(`Decrypting environment variables in ${environmentFile} with current key: ${keyName}`);
    decryptedData = await this.decryptEnvironmentVariables(keyName, environmentFile, shouldRotateKey);
    ...
    logger.info(`Re-encrypting environment variables in ${environmentFile} with new key: ${keyName}`);
    const reEncryptedCount = await this.reEncryptEnvironmentVariables(environmentFile, decryptedData, keyName);
    ...
    await this.recordPostRotationActivities(keyName, updatedMetadata, reason, rotationResult, environmentFile);
 ...
 */
private validateRotationConfigWithRepair(keyName: string, rotationConfig: KeyRotationConfig): void {
  try {
...
private async handleRotationFailure(
  ...
  if (currentMetadata) {
    await this.keyLifecycleManager.addHealthCheckEntry(
      keyName,
      currentMetadata,
      false,
      reason,
...
private validateDecryptionResults(
  ...
  // Main method - now much shorter and focused
  private async reEncryptEnvironmentVariables(
    environmentFile: string,
    decryptedVariables: Record<string, string>,
    keyName: string,
  ...
  // Reusable validation component - no hardcoded values
